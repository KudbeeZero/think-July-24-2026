...
export const ThinkTokenMeter: React.FC<ThinkTokenMeterProps> = ({
  ...
  return (
    <div 
      ...
      className={`flex items-center gap-2 sm:gap-3 bg-zinc-900/90 border ${
        isGenerating ? 'border-yellow-500/70 shadow-yellow-500/20' : 'border-zinc-800 hover:border-yellow-500/40'
      } transition-all rounded-lg px-2.5 sm:px-3 py-1.5 cursor-pointer group shadow-lg select-none`}
      title="Click to view Reasoning Terminal & Live Token Feed"
    >
      <div className="relative flex items-center justify-center">
        <Brain className={`w-4 h-4 ${isGenerating ? 'text-yellow-400 animate-pulse' : 'text-zinc-400 group-hover:text-yellow-400'} transition-transform`} />
        {isGenerating && (
          <div className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 bg-emerald-400 rounded-full animate-ping" />
        )}
      </div>

      <div className="flex flex-col min-w-[95px] sm:min-w-[110px]">
